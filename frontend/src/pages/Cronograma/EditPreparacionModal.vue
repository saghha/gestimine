<template>
  <b-modal v-model="showModal" v-if="showModal" size="xl" @hide="closeModal()" hide-footer>
    <template v-slot:modal-title>
      <strong>Editar Item Preparación</strong>
    </template>
...
export default {
  name: 'EditPreparacionCronograma',
  components: {
    VueCurrencyInput
  },
  props: {
  ...
  methods: {
    ...
    getDato: function () {
      ...
      axios.get('cronograma/preparacion/'+this.id_dato).then((response) => {
    ...
    addItem: function () {
      ...
      axios.put('cronograma/preparacion/'+this.id_dato, this.info).then((response) => {
        this.closeModal()
